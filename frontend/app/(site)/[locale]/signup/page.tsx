...
export default function SignUpPage() {
  ...
  const t = useTranslations('site.auth');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email || !password || !confirmPassword) {
      toast({ title: t('errors.fillAllFields'), variant: "error" });
      return;
    }

    if (password !== confirmPassword) {
      toast({ title: t('errors.passwordsDontMatch'), variant: "error" });
      return;
    }

    if (password.length < 6) {
      toast({ title: t('errors.passwordTooShort'), variant: "error" });
      return;
    }

    if (!agreedToTerms) {
      toast({ title: t('errors.agreeToTerms'), variant: "error" });
      return;
    }
    ...
    if (error) {
      toast({ title: t('errors.signUpFailed'), description: error.message, variant: "error" });
    } else {
      toast({ title: t('success.checkEmail'), description: t('success.checkEmailDescription'), variant: "success" });
    }
  };
        ...
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="text-center">
            <div className="flex items-center justify-center space-x-3 mb-4">
              <span className="text-2xl font-bold text-black">GESALP</span>
              ...
              <span className="text-2xl font-bold text-[#E0342C]">AI</span>
            </div>
            <h1 className="text-2xl font-bold text-black">{t('signUp.title')}</h1>
            <p className="mt-2 text-sm text-gray-600">{t('signUp.subtitle')}</p>
          </div>

          <Card className="border-gray-200 shadow-lg">
            <CardHeader>
              <CardTitle className="text-center text-lg">{t('signUp.formTitle')}</CardTitle>
            </CardHeader>
            <CardContent>
              ...
              <div className="mt-6 text-center">
                <p className="text-sm text-gray-600">
                  ...
                  <Link 
                    href={`/${locale}/signin`}
                    className="text-[#E0342C] hover:underline font-medium"
                  >
                    {t('signUp.signIn')}
                  </Link>
                </p>
