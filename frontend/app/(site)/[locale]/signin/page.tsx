...
export default function SignInPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      toast({ title: t('errors.signInFailed'), description: error.message, variant: "error" });
    } else {
      toast({ title: t('success.signedIn'), variant: "success" });
      ...
      <div className="relative flex min-h-screen items-center justify-center px-4 py-12 sm:px-6 lg:px-8">
        <div className="w-full max-w-md space-y-8">
          ...
          <div className="text-center">
            ...
            <h1 className="text-2xl font-bold text-black">{t('signIn.title')}</h1>
            <p className="mt-2 text-sm text-gray-600">{t('signIn.subtitle')}</p>
          </div>

          <Card className="border-gray-200 shadow-lg">
            <CardHeader>
              <CardTitle className="text-center text-lg">{t('signIn.formTitle')}</CardTitle>
            </CardHeader>
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex items-center justify-between">
                  <Link 
                    ...
                    className="text-sm text-[#E0342C] hover:underline"
                  >
